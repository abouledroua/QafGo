...
export default function SearchableSelect({
  ...
  const resolvedPlaceholder = placeholder || t('common.select_placeholder', 'اختر من القائمة...');
  const resolvedSearchPlaceholder = searchPlaceholder || t('common.search_placeholder', 'اكتب للبحث...');
  const resolvedEmptyMessage = emptyMessage || t('common.no_results', 'لا توجد نتائج مطابقة للبحث');
  ...
  return (
    <div ref={wrapperRef} className={`relative font-cairo ${className}`} dir="rtl">
      ...
      <button
        ...
        className={`w-full p-3 bg-surface border rounded-xl text-sm flex items-center justify-between gap-2.5 transition-all text-start ${
          isOpen 
        ...
        <div className="flex items-center gap-2.5 truncate flex-1 min-w-0">
          {selectedOption?.avatarUrl ? (
            <img 
              ...
              className="w-7 h-7 rounded-xl object-cover border border-border shadow-xs shrink-0" 
            />
          ) : selectedOption?.avatarText ? (
            <div className="w-7 h-7 rounded-xl bg-primary/10 text-primary border border-primary/20 flex items-center justify-center font-black text-xs shrink-0 shadow-xs">
              {selectedOption.avatarText}
            </div>
          ) : TriggerIcon ? (
        ...
        <div className="flex items-center gap-1 shrink-0 text-text-muted">
          {clearable && selectedOption && !disabled && (
            <span
              ...
              title={t('common.clear_selection', 'إلغاء التحديد')}
            >
        ...
        <div className={`absolute z-50 top-full mt-1.5 right-0 left-0 bg-surface-card border border-border rounded-2xl shadow-2xl overflow-hidden animate-fadeIn ${dropdownClassName}`}>
          ...
          <div className="p-2.5 border-b border-border bg-surface/40">
            <div className="relative">
              <Search className={`w-4 h-4 text-text-muted absolute top-1/2 -translate-y-1/2 pointer-events-none ${isRtl ? 'right-3' : 'left-3'}`} />
              <input
                ref={searchInputRef}
                type="text"
                value={searchTerm}
                ...
                className={`w-full ${isRtl ? 'pl-3 pr-9' : 'pr-3 pl-9'} py-2 bg-surface border border-border rounded-xl text-xs font-bold text-text-main focus:outline-none focus:border-primary placeholder:text-text-muted/70`}
              />
              {searchTerm && (
                <button
                  type="button"
                  onClick={() => setSearchTerm('')}
                  className={`absolute top-1/2 -translate-y-1/2 text-text-muted hover:text-text-main ${isRtl ? 'left-2.5' : 'right-2.5'}`}
                >
                  <X className="w-3.5 h-3.5" />
                </button>
              )}
          ...
          <div className="max-h-60 overflow-y-auto p-1.5 space-y-1 text-xs font-semibold">
            ...
            {clearable && (
              <button
                ...
                className={`w-full px-3 py-2 rounded-xl text-start transition-colors flex items-center justify-between ${
                  !value ? 'bg-primary/10 text-primary font-bold' : 'text-text-muted hover:bg-surface hover:text-text-main'
                }`}
              >
                <span>-- {t('common.unassigned', 'بدون تحديد')} --</span>
              ...
              filteredOptions.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full px-3 py-2.5 rounded-xl text-start transition-colors flex items-center justify-between gap-2.5 ${
                      isSelected
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0 flex-1 truncate">
                      {opt.avatarUrl ? (
                        ...
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs shrink-0 border shadow-xs ${
                      ...
                      <div className="flex flex-col truncate">
                        <span className="truncate leading-tight font-bold">{opt.label}</span>
                        {opt.sublabel && (
